...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::HBCUR)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("HBCUR")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizehbcurAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizehbcurAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizehbcurAmount = strAnonymizehbcurAmount.remove(strAnonymizehbcurAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizehbcurAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizehbcurAmount * COIN) nMaxToAnonymize = nAnonymizehbcurAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizehbcurAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizehbcurAmount));
        strAnonymizehbcurAmount = strAnonymizehbcurAmount.remove(strAnonymizehbcurAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizehbcurAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
